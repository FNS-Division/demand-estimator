...
from demand.utils import create_default_logger
...
class PopulationDataHandler:
    ...
    def __init__(self,
                 ...
                 worldpop_base_url='https://d35k53rhvc9u0d.cloudfront.net/worldpop_data/',
                 logger=None
                 ):
    ...
    def country_code(self, value):
        if self._is_valid_country_code(value):
            ...
            self.logger.error('Country code does not exist in the worldpop database!')
    ...
    def dataset_year(self, value):
        if self._is_valid_dataset_year(value):
            ...
            self.logger.error('Worldpop dataset for given year does not exist!')
    ...
    def get_dataset_url(self):
        ...
        if self.one_km_res:
            ...
            dataset_url[6] = dataset_url[6] + '_1km' + ('_UNadj' if self.un_adjusted else '')
            dataset_url[9] = dataset_url[9].replace(str(self.dataset_year), str(self.dataset_year) + '_1km_Aggregated')
    ...
    def download_tif_file(self):
        ...
        if not os.path.exists(self.dataset_path):
            ...
            self.logger.info('Dataset download is complete!')
    ...
    def get_dataset_xsize(self):
        ...
        """
        if not os.path.exists(self.dataset_path):
            self.logger.warn('Worldpop dataset does not exist in the directory. Downloading the tif file...')
            self.download_tif_file()

        xsize = self.data_processor.get_tif_xsixe(file_path=self.dataset_path)
    ...
    def generate_population_data(self):
        ...
        self.logger.info('Processing population tif file...')
        df_population = self.data_processor.process_tif(file_path=self.dataset_path, band_no=1, drop_nodata=True)
        df_population.rename(columns={'pixel_value': 'population'}, inplace=True)
        self.logger.info('Population tif file is processed!')
